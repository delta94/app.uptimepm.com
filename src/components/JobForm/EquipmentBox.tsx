...
import { DetailedEquipmentReference } from 'generated';
import debounce from 'lodash/debounce';
const { Option } = Select;

export interface IProps {
  data: DetailedEquipmentReference[];
  handleSearch: any;
  handleChange: any;
  loading: boolean;
  defValue: DetailedEquipmentReference[];
  placeholder: string;
}

export const EquipmentBox = (props: IProps) => {
    ...
    <>
      ...
      <List
        ...
        renderItem={item => (
          ...
          >
            <List.Item.Meta description={<div>{`${item.name} ${item.meterType}`}</div>} />
          </List.Item>
        )}
      />
    </>
