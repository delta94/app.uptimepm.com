...
import { UserReference } from 'generated';
    ...
    <>
      ...
      >
        {state.filteredData.map(item => (
          <Option key={item.id} title={item.firstName}>
      ...
      <List
        ...
        renderItem={item => (
          ...
          >
            <List.Item.Meta description={<div>{`${item.firstName} ${item.lastName}  <${item.email}>`}</div>} />
          </List.Item>
        )}
      />
    </>
