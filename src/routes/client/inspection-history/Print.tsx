  ...
  return (
    <Document>
      {inspections &&
        ...
        (inspections as Inspection[]).map((inspection: Inspection, parentIndex: number) => {
          return (
            <>
              <Page key={parentIndex} size="A4" wrap>
                {inspection && (
                  <>
                      ...
                      <View style={styles.tableNoBorder}>
                        <View style={styles.tableRow}>
                          ...
                          <View style={[styles.tableColNoBorder, styles.right, styles.paddingTopInfo]}>
                            <Text>{inspection.client && inspection.client.name}</Text>
                            <Text>Inspection #{inspection.id && inspection.id.split('/')[1]}</Text>
                          </View>
                        </View>
                      </View>
                      ...
                      <View style={[styles.tableNoBorder, styles.paddingRight]}>
                        <View style={styles.tableRow}>
                          <View style={[styles.tableColNoBorder, styles.left, styles.textSizeBig]}>
                            <Text>{inspection.equipment && inspection.equipment.name}</Text>
                          </View>
                    ...
                    <View style={[styles.tableNoBorder, styles.paddingLeft]}>
                      <View style={styles.tableRow}>
                        <View style={[styles.tableColNoBorder, styles.left, styles.textSizeBig]}>
                          <Text>Checklist Items ({inspection.checklist.length})</Text>
                        </View>
                        <View style={[styles.tableColNoBorder, styles.right]}>
                          <Text></Text>
                        </View>
                    ...
                    <View style={[styles.table, styles.marginLeft, styles.marginRight]}>
                      {inspection.checklist &&
                        inspection.checklist.map((item: InspectionChecklistItem, index: number) => {
                          return (
                            <>
                              <View key={index}>
                                <View style={styles.tableRow}>
